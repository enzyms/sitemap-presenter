...
This project uses SvelteFlow for a sitemap visualization canvas. Key files:

- **Canvas**: `apps/web/src/lib/components/canvas/SitemapCanvas.svelte` — main flow canvas with toolbar (crawl, layout, search)
- **Custom node**: `apps/web/src/lib/components/canvas/PageNode.svelte` — sitemap page nodes with screenshots, feedback badges, expand/collapse
- **Custom edge**: `apps/web/src/lib/components/canvas/LinkEdge.svelte` — floating edges for radial layout, smoothstep for hierarchical
- **Layout engine**: `apps/web/src/lib/services/layoutEngine.ts` — dagre-based layout with hierarchical (TB) and radial modes
- **State**: `apps/web/src/lib/stores/sitemap.svelte.ts` — class-based `$state` store (NOT writable), manages nodes/edges/progress/positions

### Patterns used in this project

- **One-way binding**: `nodes={sitemapStore.nodes}` (not `bind:nodes`) — store owns the state
- **NodeTypes cast**: `const nodeTypes: NodeTypes = { page: PageNode as any }` — needed for Svelte 4/5 component type mismatch
- **onviewportchange**: Not in `@xyflow/svelte` types but works at runtime — use spread to bypass: `{...({ onviewportchange: handler } as any)}`
- **Node drag persistence**: `onnodedragstop` handler saves positions to localStorage per site+layout mode
- **Layout modes**: "hierarchical" (dagre TB) and "radial" (circular) — user can switch, positions saved separately per mode
- **Immutable updates**: Store methods always create new arrays/objects (e.g., `this.nodes = this.nodes.map(...)`)
